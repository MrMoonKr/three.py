...
class TestTrackballControls(Base):

    def initialize(self):

        self.setWindowTitle("Trackball Controls")
        self.setWindowSize(1200, 760)
        self.centerWindow()

        self.renderer = Renderer()
        self.renderer.setViewportSize(1200, 760)
        self.renderer.setClearColor(0.08, 0.09, 0.12)
        ...
        self.scene.add(AmbientLight(color=[1, 1, 1], strength=0.35))

        self.pointLight = PointLight(color=[1, 1, 1], position=[4, 5, 3], strength=0.8)
        self.scene.add(self.pointLight)
        self.scene.add(PointLightHelper(self.pointLight, radius=0.15))

        floorMesh = GridHelper(size=20, divisions=20, gridColor=[0.2, 0.2, 0.2], centerColor=[0.8, 0.2, 0.2])
    ...
    def update(self):
        ...
        self.pointLight.transform.rotateY(0.015, Matrix.GLOBAL)
        self.pointLight.transform.rotateX(-0.006, Matrix.GLOBAL)
...
class GLApp(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.base = TestTrackballControls(self)
